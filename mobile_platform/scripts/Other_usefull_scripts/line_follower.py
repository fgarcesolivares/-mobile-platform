...
class LineFollower(object):
    def __init__(self, rgb_to_track, colour_error_perc = 10.0,colour_cal=False, camera_topic="/camera1/image_raw", cmd_vel_topic="/cmd_vel"):
        ...
        rclpy.logwarn("OpenCV Version Installed==>"+str(self.major))

        # This way we process only half the frames
        self.process_this_frame = True

        self.bridge_object = CvBridge()
        self.image_sub = rclpy.Subscriber(camera_topic, Image, self.camera_callback)
        self.cmd_vel_pub = rclpy.Publisher(cmd_vel_topic, Twist, queue_size=1)
    ...
    def camera_callback(self, data):
        ...
        if self.process_this_frame:
            ...
            rclpy.loginfo("height=%s, width=%s" % (str(height), str(width)))
            ...
            rclpy.loginfo("Number of centroids==>" + str(len(contours)))
            centres = []
            ...
            for candidate in centres:
                ...
                try:
                    ...
                    rclpy.logwarn("Centroid FOUND ==" + str(cx) + "," + str(cy) + "")
                except:
...
if __name__ == '__main__':
    rclpy.init_node('line_follower_start', anonymous=True)
